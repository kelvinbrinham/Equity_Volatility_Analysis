...
for i in range(len(stock_df_lst)):
    ...
    stock_letter_df['Date'] = stock_letter_df['Date'] - \
        stock_letter_df['Date'].shift()
    ...
    for q in range(len(stock_letter_df_chunk_lst)):
        ...
        # 5. Calc. 30-minute average 5-minute return
        stock_letter_df_chunk_resample['30-minute avg 5-minute return'] = stock_letter_df_chunk_resample['5-Minute (log) Return'].rolling(6).mean()
        stock_letter_df_chunk_resample = stock_letter_df_chunk_resample.drop(['5-Minute (log) Return', 'price'], axis = 1)
        ...
        if len(stock_letter_df_chunk_resample) != 15:
            ...
            new_first_line = pd.DataFrame([[df_['ts'][0] - timedelta(minutes=30), df_['volume'].mean(), df_['30-minute avg 5-minute return'].mean()]], columns=df_.columns)
